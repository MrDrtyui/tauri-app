...
export function GraphPanel() {
  ...
  const handleNodeMouseDown = useCallback(
    (e: React.MouseEvent, nodeId: string) => {
      ...
      draggingNode.current = {
        id: nodeId,
        offX: e.clientX - (pos.x * zoom + pan.x + canvasRect.left),
        offY: e.clientY - (pos.y * zoom + pan.y + canvasRect.top),
      };
      let lastPos = { ...pos };
    ...
    [storeNodes, pan, zoom, localPos, updateNodePosition]
  );
  ...
  const handleNodeClick = useCallback(
    (e: React.MouseEvent, node: YamlNode) => {
      ...
      setSelectedEntity({ type: "graphNode", id: node.id, label: node.label, filePath: node.file_path,
        meta: { typeId: node.type_id, kind: node.kind, namespace: node.namespace, source: node.source, replicas: node.replicas, image: node.image } });
      if (node.file_path) {
        const fileName = node.file_path.split("/").pop() ?? node.label;
        openTab({ id:`file-${node.file_path}`, title:fileName, contentType:"file", filePath:node.file_path, icon: node.file_path.includes("Chart.yaml")?"⛵":"📄" }, "center");
      }
    },
    [setSelectedEntity, openTab]
  );
  ...
  const handleWheel = (e: React.WheelEvent) => { e.preventDefault(); setZoom((z) => Math.max(0.25, Math.min(3, z * (e.deltaY < 0 ? 1.1 : 0.9)))); };
  ...
  return (
    <div style={{ position:"relative", width:"100%", height:"100%", overflow:"hidden", background:"#0a1628" }} onWheel={handleWheel}>
      <div className="canvas-bg" ref={canvasRef} onMouseDown={handleCanvasMouseDown}
        style={{ position:"absolute", inset:0, backgroundImage:"linear-gradient(rgba(59,130,246,0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(59,130,246,0.05) 1px, transparent 1px)",
          backgroundSize:`${40*zoom}px ${40*zoom}px`, backgroundPosition:`${pan.x}px ${pan.y}px`, cursor:"grab" }} />

      {storeNodes.length === 0 && (
        <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
          color:"rgba(255,255,255,0.15)", fontFamily:"monospace", pointerEvents:"none", gap:8 }}>
      ...
      <div style={{ position:"absolute", top:10, right:10, zIndex:10, display:"flex", gap:4 }}>
        {[{ l:"⊡", t:"Fit view", a:fitView }, { l:"+", t:"Zoom in", a:()=>setZoom(z=>Math.min(3,z*1.2)) },
          { l:"−", t:"Zoom out", a:()=>setZoom(z=>Math.max(0.25,z/1.2)) }, { l:"⊞", t:"Reset", a:()=>{setPan({x:0,y:0});setZoom(1);} }
        ].map((btn) => (
          <button key={btn.l} onClick={btn.a} title={btn.t} style={{ width:28,height:28,background:"rgba(13,22,40,0.85)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:5,color:"rgba(255,255,255,0.5)",cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(8px)" }}
            onMouseEnter={(e)=>(e.currentTarget.style.color="white")} onMouseLeave={(e)=>(e.currentTarget.style.color="rgba(255,255,255,0.5)")}>{btn.l}</button>
        ))}
      ...
      <div style={{ position:"absolute",inset:0,transformOrigin:"0 0",transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`,pointerEvents:"none" }}>
        {storeNodes.map((node) => {
          ...
          return (
            <div key={node.id} onMouseDown={(e) => handleNodeMouseDown(e, node.id)} onClick={(e) => handleNodeClick(e, node)}
              style={{ position:"absolute", left:pos.x, top:pos.y, width:155, padding:"7px 10px", borderRadius:7, background:t.bg,
                border:`1.5px solid ${isSelected?t.accent:t.border}`, color:t.color, fontSize:11, fontFamily:"monospace", fontWeight:600,
                cursor:"grab", userSelect:"none", pointerEvents:"all",
                boxShadow: isSelected?`0 0 0 2px ${t.accent}, 0 0 20px ${t.shadow}`:`0 0 10px ${t.shadow}`,
                transition:"box-shadow 0.12s, border-color 0.12s", display:"flex", alignItems:"center", gap:6 }}>
      ...
      <div style={{ position:"absolute", bottom:8, left:"50%", transform:"translateX(-50%)", color:"rgba(255,255,255,0.1)", fontSize:10, fontFamily:"monospace", pointerEvents:"none" }}>
        scroll to zoom · drag canvas to pan · drag nodes — positions saved to .endfield
      </div>
    </div>
  );
}
